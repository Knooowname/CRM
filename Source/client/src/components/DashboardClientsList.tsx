                ...
                <tbody className="flex flex-col gap-4 max-h-full">
                    {clients?.map((item, index) => (
                        <tr key={index} className="w-full h-20 flex items-center justify-between py-2 px-4 border-1 border-gray-300 rounded-md">
                            <DashboardClientsCard setCurrentUserId={setCurrentUserId ? setCurrentUserId : undefined} id={item.id} img={''} name={item.first_name} surname={item.last_name} recordingTime={item.date_create} status={'Need approve'} />
                        </tr>
                    ))}
                </tbody>
            </table>
